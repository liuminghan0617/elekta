 ...
 * text\hidden\password\textarea\radio\checkbox\select
 * file只是其值（路径）获取和回填，涉及安全问题，不适合进行JS操作，如需要用到FILE，请为FILE做单独处理。
 * 主要函数：
 * bindForm(formId, data,suffix)			将JSON数据填充到FORM中
 * formToObjParms(formId,params,suffix)		form元素转为对象数组
 * formToJsonParms(formId,params,suffix)	form元素转为JSON对象
 * formToUrlParams(formId,params,suffix)	form元素转为URL参数
 * resetForm(formId)						重置表单
 * cleanForm(formId)						清空表单
 */
 ...
 * 
 * @param formId: Form的Id
 * @param urlParams: 需要添加至的URL参数字符串，可为空。
...
function cleanForm(formId){
	var $form = $("#"+formId);
	$form.find("input:text,input[type='hidden'],input:password,textarea,input:file").val("");
	var $radio = $form.find("input:radio:checked");
 ...
 * 单选钮插件赋值方法
 * 
 * @param radioId(radio的ID或NAME)
 */
function setRadioValue(radioId){
	var $radio = $(":radio[name=" + radioId + "]");
	var $radiop = $radio.parent();
	var $val = $(":radio[name=" + radioId + "]:checked");
	$radiop.find(".radio0").removeClass("radiocheck").addClass("radio");
	if($val && $val.length>0){
		$val.next(".radio0").removeClass("radio").addClass("radiocheck");
	}
}
